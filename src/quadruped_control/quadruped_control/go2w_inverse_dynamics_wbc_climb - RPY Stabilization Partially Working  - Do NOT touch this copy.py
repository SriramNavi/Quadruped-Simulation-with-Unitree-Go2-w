...
class WbcConfig:
    ...
    fallback_pose_weight: float = 0.5
    maximum_slope_disagreement_deg: float = 5.0
    maximum_slope_rate_degps: float = 20.0
    minimum_local_slope_progress_m: float = 0.01
    maximum_local_slope_rate_degps: float = 25.0
    local_slope_filter_alpha: float = 0.2
    maximum_front_rear_slope_difference_deg: float = 12.0
    ...
    def validate(self) -> None:
        ...
        positive = {
            ...
            'maximum_slope_disagreement_deg': (
                self.maximum_slope_disagreement_deg
            ),
            'maximum_slope_rate_degps': self.maximum_slope_rate_degps,
            'minimum_local_slope_progress_m': (
                self.minimum_local_slope_progress_m
            ),
            'maximum_local_slope_rate_degps': (
                self.maximum_local_slope_rate_degps
            ),
            'maximum_front_rear_slope_difference_deg': (
                self.maximum_front_rear_slope_difference_deg
            ),
        }
        ...
        if not isinstance(self.hybrid_leveling_enabled, bool):
            raise ValueError('hybrid_leveling_enabled must be boolean')
        nonnegative = {
        ...
        if not 0.0 <= self.fallback_pose_weight <= 1.0:
            raise ValueError('fallback_pose_weight must be in [0, 1]')
        if not 0.0 < self.local_slope_filter_alpha <= 1.0:
            raise ValueError('local_slope_filter_alpha must be in (0, 1]')
...
@dataclass(frozen=True)
class TerrainSlopeDiagnostics:
    """Separated pose, wheel-center, fused, and axle-local slope estimates."""

    wheel_center_slope_deg: float = 0.0
    pose_slope_deg: float = 0.0
    fused_slope_deg: float = 0.0
    front_local_slope_deg: float = 0.0
    rear_local_slope_deg: float = 0.0
    front_local_slope_valid: bool = False
    rear_local_slope_valid: bool = False
...
class ContactEstimator:
    ...
    def __init__(self, config: WbcConfig, wheel_radius: float) -> None:
        ...
        self.slope_diagnostics = TerrainSlopeDiagnostics()
        self.previous_valid_fused_slope_deg = 0.0
        self.axle_slope_histories: dict[
            str, list[tuple[float, np.ndarray]]
        ] = {'front': [], 'rear': []}
        self.axle_slope_update_stamps: dict[str, float] = {}
        self.axle_local_slopes = {'front': 0.0, 'rear': 0.0}
        self.axle_local_valid = {'front': False, 'rear': False}
    ...
    def reset(self, state: EstimatedState) -> None:
        ...
        self.slope_diagnostics = TerrainSlopeDiagnostics()
        self.previous_valid_fused_slope_deg = 0.0
        self.axle_slope_histories = {'front': [], 'rear': []}
        self.axle_slope_update_stamps.clear()
        self.axle_local_slopes = {'front': 0.0, 'rear': 0.0}
        self.axle_local_valid = {'front': False, 'rear': False}

    @staticmethod
    def wheelbase_heading_and_slope(
        front_center: np.ndarray,
        rear_center: np.ndarray,
        fallback_heading: np.ndarray,
    ) -> tuple[np.ndarray, float]:
        """Return current horizontal wheelbase heading and posture slope."""
        separation = np.asarray(front_center) - np.asarray(rear_center)
        horizontal_separation = separation.copy()
        horizontal_separation[2] = 0.0
        horizontal_norm = float(np.linalg.norm(horizontal_separation))
        if horizontal_norm > 1.0e-9:
            heading = horizontal_separation / horizontal_norm
        else:
            heading = np.asarray(fallback_heading, dtype=float).copy()
            heading[2] = 0.0
        ...
        forward_separation = float(separation @ heading)
        slope = math.degrees(math.atan2(
            float(separation[2]), max(0.05, forward_separation),
        ))
        if not finite(heading, forward_separation, slope):
            raise ValueError('INVALID_WHEELBASE_SLOPE_GEOMETRY')
        return heading, slope

    @staticmethod
    def symmetric_fused_slope(
        wheel_center_slope_deg: float,
        pose_slope_deg: float,
        previous_fused_slope_deg: float,
        pose_valid: bool,
        config: WbcConfig,
        dt_sec: float,
    ) -> float:
        """Blend pose and posture symmetrically, then bound temporal change."""
        candidate = wheel_center_slope_deg
        if pose_valid:
            difference = clamp(
                pose_slope_deg - wheel_center_slope_deg,
                -config.maximum_slope_disagreement_deg,
                config.maximum_slope_disagreement_deg,
            )
            candidate += config.fallback_pose_weight * difference
        candidate = clamp(candidate, 0.0, config.maximum_slope_deg)
        fused = rate_limit(
            previous_fused_slope_deg, candidate,
            config.maximum_slope_rate_degps, dt_sec,
        )
        return clamp(fused, 0.0, config.maximum_slope_deg)

    def _update_axle_local_slope(
        self,
        axle: str,
        stamp_sec: float,
        center: np.ndarray,
        ...
        """Update one axle slope using bounded-history linear regression."""
        center = np.asarray(center, dtype=float)
        if not finite(stamp_sec, center, heading):
            return
        history = self.axle_slope_histories[axle]
        if history and stamp_sec <= history[-1][0]:
            return
        history.append((stamp_sec, center.copy()))
        oldest_stamp = stamp_sec - 1.0
        history[:] = [sample for sample in history if sample[0] >= oldest_stamp]
        if len(history) > 120:
            del history[:-120]
        if len(history) < 2:
            return
        origin = history[0][1]
        progress = np.asarray([
            float((sample_center - origin) @ heading)
            for _, sample_center in history
        ])
        height = np.asarray([
            float(sample_center[2]) for _, sample_center in history
        ])
        if (
            progress[-1] - progress[0]
            < self.config.minimum_local_slope_progress_m
        ):
            return
        centered_progress = progress - float(np.mean(progress))
        denominator = float(centered_progress @ centered_progress)
        if denominator <= 1.0e-12:
            return
        gradient = float(
            centered_progress @ (height - float(np.mean(height)))
        ) / denominator
        raw_slope = math.degrees(math.atan(gradient))
        if not math.isfinite(raw_slope):
            return
        raw_slope = clamp(raw_slope, 0.0, self.config.maximum_slope_deg)
        previous = self.axle_local_slopes[axle]
        filtered = (
            self.config.local_slope_filter_alpha * raw_slope
            + (1.0 - self.config.local_slope_filter_alpha) * previous
        )
        old_update_stamp = self.axle_slope_update_stamps.get(axle)
        dt_sec = (
            stamp_sec - history[0][0]
            if old_update_stamp is None else stamp_sec - old_update_stamp
        )
        if dt_sec <= 1.0e-6:
            return
        updated = rate_limit(
            previous, filtered,
            self.config.maximum_local_slope_rate_degps, dt_sec,
        )
        other_axle = 'rear' if axle == 'front' else 'front'
        if self.axle_local_valid[other_axle]:
            other_slope = self.axle_local_slopes[other_axle]
            disagreement = self.config.maximum_front_rear_slope_difference_deg
            updated = clamp(
                updated, other_slope - disagreement, other_slope + disagreement,
            )
        self.axle_local_slopes[axle] = clamp(
            updated, 0.0, self.config.maximum_slope_deg,
        )
        self.axle_slope_update_stamps[axle] = stamp_sec
        self.axle_local_valid[axle] = True
    ...
    def update_terrain(
        ...
        wheelbase_heading, wheel_center_slope = self.wheelbase_heading_and_slope(
            front_center, rear_center, self.heading,
        )
        wheel_center_slope = clamp(
            wheel_center_slope, 0.0, self.config.maximum_slope_deg,
        )
        pose_valid = raw.valid and finite(
            raw.fast_slope_deg, raw.stable_slope_deg,
        )
        pose_slope = (
            clamp(raw.stable_slope_deg, 0.0, self.config.maximum_slope_deg)
            if pose_valid else self.slope_diagnostics.pose_slope_deg
        )
        dt_sec = (
            1.0 / self.config.control_rate_hz
            if self.last_pose_stamp is None
            else state.stamp_sec - self.last_pose_stamp
        )
        fallback_fused_slope = self.symmetric_fused_slope(
            wheel_center_slope, pose_slope,
            self.previous_valid_fused_slope_deg, pose_valid,
            self.config, dt_sec,
        )
        self._update_axle_local_slope(
            'front', state.stamp_sec, front_center, wheelbase_heading,
        )
        self._update_axle_local_slope(
            'rear', state.stamp_sec, rear_center, wheelbase_heading,
        )
        valid_local_slopes = [
            self.axle_local_slopes[axle]
            for axle in ('front', 'rear')
            if self.axle_local_valid[axle]
        ]
        if valid_local_slopes:
            local_candidate = float(np.mean(valid_local_slopes))
            fused_slope = rate_limit(
                self.previous_valid_fused_slope_deg, local_candidate,
                self.config.maximum_slope_rate_degps, dt_sec,
            )
            fused_slope = clamp(
                fused_slope, 0.0, self.config.maximum_slope_deg,
            )
        else:
            fused_slope = fallback_fused_slope
        self.previous_valid_fused_slope_deg = fused_slope
        self.slope_diagnostics = TerrainSlopeDiagnostics(
            wheel_center_slope_deg=wheel_center_slope,
            pose_slope_deg=pose_slope,
            fused_slope_deg=fused_slope,
            front_local_slope_deg=self.axle_local_slopes['front'],
            rear_local_slope_deg=self.axle_local_slopes['rear'],
            front_local_slope_valid=self.axle_local_valid['front'],
            rear_local_slope_valid=self.axle_local_valid['rear'],
        )
        local_slopes = [
            self.axle_local_slopes[axle]
            for axle in ('front', 'rear')
        ...
        terrain_signal = max(local_slopes) if local_slopes else fused_slope
        progress = float(
            (state.base_position - self.origin_position) @ self.heading
        )
        ...
        phase = raw.contact_phase
        if (
            phase != ContactPhase.FLAT_APPROACH
            and terrain_signal < self.terrain_estimator.config.entry_slope_deg
            and not crest_context
        ):
            _, _, yaw = quaternion_to_rpy(state.base_quaternion)
            self.terrain_estimator.reset(*state.base_position, yaw)
            phase = ContactPhase.FLAT_APPROACH
        ...
        self.terrain = TerrainEstimate(
            ...
            slope_confidence=raw.slope_confidence,
            slope_rate_degps=(
                fused_slope - self.terrain.fast_slope_deg
            ) / max(dt_sec, 1.0e-6),
            transition_probability=raw.transition_probability,
            contact_phase=phase,
            valid=raw.valid or finite(wheel_center_slope),
            stale=False,
        )
        return self.terrain
    ...
    def slope_for_leg(self, leg: str, terrain: TerrainEstimate) -> float:
        """Prefer the relevant axle trajectory, then the bounded fused slope."""
        front = leg.startswith('F')
        local_slope = (
            self.slope_diagnostics.front_local_slope_deg
            if front else self.slope_diagnostics.rear_local_slope_deg
        )
        local_valid = (
            self.slope_diagnostics.front_local_slope_valid
            if front else self.slope_diagnostics.rear_local_slope_valid
        )
        if local_valid and math.isfinite(local_slope):
            selected = local_slope
        elif math.isfinite(self.slope_diagnostics.fused_slope_deg):
            selected = self.slope_diagnostics.fused_slope_deg
        elif math.isfinite(self.previous_valid_fused_slope_deg):
            selected = self.previous_valid_fused_slope_deg
        elif terrain.contact_phase == ContactPhase.FLAT_APPROACH:
            selected = 0.0
        else:
            selected = max(terrain.fast_slope_deg, terrain.stable_slope_deg)
        return math.radians(clamp(selected, 0.0, self.config.maximum_slope_deg))
...
class HybridPositionVelocityAdapter:
    ...
    def _leveling_orientation_correction(
        ...
        """Rate-limit the bounded SO(3) correction used by hybrid leg IK."""
        ...
        self.last_leveling_orientation_body += change
        if not finite(self.last_leveling_orientation_body):
            self.last_leveling_orientation_body[:] = 0.0
        return self.last_leveling_orientation_body.copy()

    def _leveling_joint_correction(
        self,
        dynamics: DynamicsSnapshot,
        contacts: Mapping[str, ContactFrame],
        target: PostureTarget,
    ) -> np.ndarray:
        """Map body leveling to bounded contact-fixed leg displacements."""
        delta_theta_body = self._leveling_orientation_correction(
            dynamics, target,
        )
        desired = np.zeros(12)
        damping_squared = self.config.hybrid_leveling_damping ** 2
        for leg_index, leg in enumerate(LEGS):
            frame = contacts.get(leg)
            if frame is None:
                self.leveling_solve_fallbacks += 1
                continue
            jacobian = np.asarray(frame.point_jacobian, dtype=float)
            velocity_indices = [
                self.robot.v_indices[f'{leg}_{joint}_joint']
                for joint in ('hip', 'thigh', 'calf')
            ]
            if (
                jacobian.ndim != 2 or jacobian.shape[0] != 3
                or jacobian.shape[1] <= max(velocity_indices)
                or not finite(jacobian)
            ):
                self.leveling_solve_fallbacks += 1
                continue
            # LOCAL_WORLD_ALIGNED changes the spatial output frame only. The
            # free-flyer tangent columns still consume body-frame increments.
            base_rotation_jacobian = jacobian[:, 3:6]
            leg_jacobian = jacobian[:, velocity_indices]
            rhs = -(base_rotation_jacobian @ delta_theta_body)
            try:
                solution = leg_jacobian.T @ np.linalg.solve(
                ...
                self.leveling_solve_fallbacks += 1
                continue
            if not finite(solution):
                self.leveling_solve_fallbacks += 1
                continue
            start = 3 * leg_index
            desired[start:start + 3] = solution
        maximum = self.config.maximum_leveling_joint_correction_rad
        desired = np.clip(desired, -maximum, maximum)
        step = self.config.maximum_leveling_joint_step_rad
        updated = self.last_leveling_correction + np.clip(
            desired - self.last_leveling_correction, -step, step,
        )
        self.last_leveling_correction = np.clip(
            updated, -maximum, maximum,
        )
        return self.last_leveling_correction.copy()
    ...
    def propose(
        ...
        self.last_wbc_joint_delta = raw - positions
        if dynamics is not None and contacts is not None and target is not None:
            raw += self._leveling_joint_correction(
                dynamics, contacts, target,
            )
        else:
            self.last_leveling_orientation_body[:] = 0.0
...
class RunLogger:
    ...
    FIELDS = (
        ...
        'terrain_wheel_center_slope_deg', 'terrain_pose_slope_deg',
        'terrain_fused_slope_deg', 'terrain_front_local_slope_deg',
        'terrain_rear_local_slope_deg', 'terrain_front_local_valid',
    ...
    def __init__(self, config: WbcConfig, backend_label: str) -> None:
        ...
        self.wheel_center_fused_differences: list[float] = []
        self.pose_fused_differences: list[float] = []
        self.front_rear_differences: list[float] = []
        self.terrain_samples = 0
        self.front_local_valid_samples = 0
        self.rear_local_valid_samples = 0
    ...
    def write(self, row: Mapping[str, Any]) -> None:
        wheel_center = float(row.get('terrain_wheel_center_slope_deg', 0.0))
        pose_slope = float(row.get('terrain_pose_slope_deg', 0.0))
        fused_slope = float(row.get('terrain_fused_slope_deg', 0.0))
        front_slope = float(row.get('terrain_front_local_slope_deg', 0.0))
        rear_slope = float(row.get('terrain_rear_local_slope_deg', 0.0))
        if finite(
            wheel_center, pose_slope, fused_slope, front_slope, rear_slope,
        ):
            self.wheel_center_fused_differences.append(
                abs(wheel_center - fused_slope)
            )
            self.pose_fused_differences.append(abs(pose_slope - fused_slope))
            self.front_rear_differences.append(abs(front_slope - rear_slope))
            self.terrain_samples += 1
            self.front_local_valid_samples += int(bool(
                row.get('terrain_front_local_valid', False)
            ))
            self.rear_local_valid_samples += int(bool(
                row.get('terrain_rear_local_valid', False)
            ))
        ...
        for name in (
            ...
            'desired_torque', 'wbc_joint_delta_rad',
    ...
    def finalize(
        ...
        if self.terrain_samples == 0:
            terrain_estimator_result = 'NO_SAMPLES'
        elif not (
            self.front_local_valid_samples or self.rear_local_valid_samples
        ):
            terrain_estimator_result = 'FALLBACK_ONLY'
        elif (
            self.front_local_valid_samples == self.terrain_samples
            and self.rear_local_valid_samples == self.terrain_samples
        ):
            terrain_estimator_result = 'AXLE_LOCAL_VALID'
        else:
            terrain_estimator_result = 'AXLE_LOCAL_PARTIAL'
        ...
        summary = {
            ...
            'maximum_abs_imu_pitch_deg': max(
                (abs(v) for v in self.pitch_values), default=None,
            ),
            ...
            'maximum_wheel_center_minus_fused_slope_deg': max(
                self.wheel_center_fused_differences, default=None,
            ),
            'maximum_pose_minus_fused_slope_deg': max(
                self.pose_fused_differences, default=None,
            ),
            'maximum_front_rear_slope_difference_deg': max(
                self.front_rear_differences, default=None,
            ),
            'mean_abs_front_rear_slope_difference_deg': mean(
...
class Go2WInverseDynamicsWbcNode(Node):
    ...
    def _create_ros_interfaces(self) -> None:
        ...
        self.float_publishers = {
            ...
            for name, topic in {
                ...
                'fused_slope': 'terrain/fused_slope_deg',
    ...
    def _publish_monitoring(
        ...
        floats = {
            ...
            'fused_slope': slope_diagnostics.fused_slope_deg,
    ...
    def _log_cycle(
        ...
        self.logger.write({
            ...
            'terrain_wheel_center_slope_deg': (
                slope_diagnostics.wheel_center_slope_deg
            ),
            'terrain_pose_slope_deg': slope_diagnostics.pose_slope_deg,
            'terrain_fused_slope_deg': slope_diagnostics.fused_slope_deg,
            'terrain_front_local_slope_deg': (
                slope_diagnostics.front_local_slope_deg
            ),
            'terrain_rear_local_slope_deg': (
                slope_diagnostics.rear_local_slope_deg
            ),
            'terrain_front_local_valid': (
                slope_diagnostics.front_local_slope_valid
            ),
            'terrain_rear_local_valid': (
                slope_diagnostics.rear_local_slope_valid
            ),
            'imu_pitch_deg': math.degrees(pitch),
    ...
    def _control_cycle(self) -> None:
        ...
        if now_wall - self.last_status_wall >= self.config.status_period_sec:
            ...
            self.get_logger().info(
                ...
                f'imu_rpy=({math.degrees(roll):.1f},'
                f'{math.degrees(pitch):.1f},{math.degrees(yaw):.1f})deg '
                f'target_rpy=({math.degrees(target.desired_roll_rad):.1f},'
                f'{math.degrees(target.desired_pitch_rad):.1f},'
                f'{math.degrees(target.desired_yaw_rad):.1f})deg '
