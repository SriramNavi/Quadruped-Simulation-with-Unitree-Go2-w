...
def generate_launch_description():
    ...
    declare_use_champ = DeclareLaunchArgument(
        "use_champ",
        default_value="true",
        description="Launch CHAMP gait/control nodes when true.",
    )
    declare_use_rendering_sensors = DeclareLaunchArgument(
    ...
    quadruped_controller_node = Node(
        ...
        condition=IfCondition(use_champ),
    )
    ...
    gz_sim = IncludeLaunchDescription(
        ...
        launch_arguments={
            'gz_args': [gazebo_world, ' -r']  # Add -r flag to start unpaused
        }.items(),
    ...
    gz_sim_headless = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_gz_sim, 'launch', 'gz_sim.launch.py')),
        launch_arguments={
            'gz_args': [gazebo_world, ' -r -s --headless-rendering']
        }.items(),
